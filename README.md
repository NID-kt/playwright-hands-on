...
`e2e` ディレクトリーに、`home.test.ts` というファイルを作成して、以下のコードを記述してください。
...
このテストコードは、フォームの入力と送信を行い、送信完了メッセージが表示されることを確認するテストです。
...
## VRT（Visual Regression Testing）の追加

VRT（Visual Regression Testing）を行うためのテストコードを追加します。

`e2e` ディレクトリーに、`home.vrt.ts` というファイルを作成して、以下のコードを記述してください。
